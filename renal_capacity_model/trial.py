    ...
    def run_trial(self):
        for run in range(self.config.number_of_runs):
            ...
            results_grouped_by_age = (
                ...
                .agg({"diverted_to_con_care": "sum", "entry_time": "count","suitable_for_transplant":"sum","live_transplant":"sum","cadaver_transplant":"sum","pre_emptive_transplant":"sum"})
                .rename(columns={"entry_time": "total_entries"})
            )
            self.df_trial_results.loc[run, "total_entries"] = (
            ...
            self.df_trial_results.loc[run,"proportion_live_transplant"] = (
                results_grouped_by_age["live_transplant"].sum()
                / (results_grouped_by_age["live_transplant"].sum() +results_grouped_by_age["cadaver_transplant"].sum())  
            )
            for age_group in results_grouped_by_age.index:
                self.df_trial_results.loc[
                    run, f"live_transplants_{int(age_group)}"
                ] = (
